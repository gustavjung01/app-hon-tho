...
function normalizeMessage(message: RuntimeMessage) {
  if (message.role === "tool") return { role: "user", content: `[Dữ liệu công cụ]\n${message.content}` };
  if (message.role === "system") return { role: "user", content: `[Ghi chú hệ thống]\n${message.content}` };
...
export function buildAgentSystemPrompt(input: AiRuntimeInput) {
  ...
  return `${input.agent.system_prompt}${appRun}${extra}`;
}
...
function stubResponse(input: AiRuntimeInput, systemPrompt: string, provider: string, note: string): AiRuntimeResult {
  ...
  return {
    ...
    tokenOutput,
    cost: 0,
    requestJson: { mode: "stub", provider, appKey: input.agent.app_key, agentId: input.agent.id, model: input.agent.model, note },
...
export async function runAgentRuntime(input: AiRuntimeInput): Promise<AiRuntimeResult> {
  ...
  if (agentProvider === "dialogflow_cx") {
    return stubResponse(input, systemPrompt, "dialogflow_cx", "Dialogflow CX runtime chưa hoàn tất cấu hình gọi thật Google.");
  }

  if (!isOpenAiProvider(agentProvider)) {
    return stubResponse(input, systemPrompt, agentProvider, `Provider ${agentProvider} chưa được runtime hỗ trợ.`);
  }
  ...
  const requestJson = {
    model,
    temperature: asNumber(input.agent.temperature, 0.3),
    max_tokens: Number(input.agent.max_tokens || 1200),
    messages: [{ role: "system", content: systemPrompt }, ...input.messages.map(normalizeMessage)]
  };
  ...
  if (!response.ok) throw new Error(responseJson?.error?.message || response.statusText || "AI provider error");
